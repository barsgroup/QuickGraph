namespace QuickGraph.Tests.Algorithms.ShortestPath
{
    ...
    public class BoostFloydWarshallTest
    {
        ...
        public void Compute()
        {
            ...
            {
                double distance;
                Assert.True(fw.TryGetDistance('A', 'A', out distance));
                Assert.Equal(0, distance);

                Assert.True(fw.TryGetDistance('A', 'B', out distance));
                Assert.Equal(6, distance);

                Assert.True(fw.TryGetDistance('A', 'C', out distance));
                Assert.Equal(1, distance);

                Assert.True(fw.TryGetDistance('A', 'D', out distance));
                Assert.Equal(4, distance);

                Assert.True(fw.TryGetDistance('A', 'E', out distance));
    ...
    public class FloydDijkstraCompareTest
    {
        ...
        {
            Assert.True(fwedges[0].Source.Equals(source));
            for (var i = 0; i < fwedges.Length - 1; ++i)
                Assert.Equal(fwedges[i].Target, fwedges[i + 1].Source);
            Assert.True(fwedges[fwedges.Length - 1].Target.Equals(target));
        }

        private void Compare<TVertex, TEdge, TGraph>(
        ...
        private static void DumpPaths<TVertex, TEdge>(TVertex source, TVertex target, TEdge[] fwedges, TEdge[] dijedges) where TEdge : IEdge<TVertex>
        {
            ...
            for (var j = 0; j < dijedges.Length; ++j)
                TestConsole.WriteLine("\t{0}", dijedges[j]);
            TestConsole.WriteLine("floyd:");
            for (var j = 0; j < fwedges.Length; ++j)
